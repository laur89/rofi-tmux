...
class Listener(object):
    ...
    def run(self):
        ...
        threads.append(Thread(target=self._server.serve_forever))   #TODO <-- see https://www.programcreek.com/python/example/105688/xmlrpc.server.SimpleXMLRPCServer
        for t in threads:
            t.start()
    ...
    def _register_cur_sessions(self) -> None:
        ...
        print('sess registr took {}'.format(time.time() - start_time))
    ...
    def _rofi_tmux_window(self, action, session_name, global_scope,
        ...
        if session_name:
            ...
            if session:
                if global_scope:
                    windows = []
                    # self._libts._list_windows()
                    print('  -> kek took {}'.format(time.time() - start_time))
                    for s in self._sessions:
        ...
        if windows:
            ...
            print('  -> lek3 took {}'.format(time.time() - start_time))
            is_tmux_win_visible = False
            start_time = time.time()
            ...
            if self._wm:
                ...
                self.logger.debug('resolving is_tmux_win_visible...')
                is_tmux_win_visible = self._wm.is_tmux_win_visible(self._cur_tmux_s)
                print('i3wm is_vis took {}'.format(time.time() - start_time))
                self.logger.debug('is_tmux_win_visible: {}'.format(is_tmux_win_visible))
            try:
                if is_tmux_win_visible:
            ...
            print('   pre ROfI {}'.format(time.time() - _start_time))
